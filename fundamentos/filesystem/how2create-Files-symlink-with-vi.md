...
O `vi` é um editor de texto poderoso e orientado para tela. Ele tem dois modos principais: modo de comando e modo de inserção. Você começa no modo de comando, onde as pressionamentos de tecla são interpretados como comandos. Para digitar texto, você deve entrar no modo de inserção.
...
2. Use o comando `vi` seguido do nome do arquivo para criar um novo arquivo. Por exemplo, vamos criar um arquivo chamado 'file1':
   ...
   vi file1
   ```
3. Dentro do ´vi´, você estará no modo de comando. Para entrar no modo de inserção e começar a digitar, pressione a tecla `i`, agora aparece o nome "--Inserção--" ou "--INSERT--" abaixo onde aparece os comandos. Agora você pode digitar o conteúdo do arquivo. Por exemplo, digite "Hello, World!".

![exemplo da inserção](../imgs/exemplo-insertmode-with-vi.png "exemplo no editor vi")

4. Para salvar e sair do arquivo, você vai pressionar a tecla `Esc` para voltar ao modo de comando, e depois segura a tecla `shift` enquanto clica duas vezes na letra `Z` (ZZ). Isso salva o arquivo e fecha o editor. Confere se o arquivo foi criado:
 ```bash
ls -l file1
   ```
...
ln -s file1 filelink
```
5.1. Precisamos testar os resultados. use o comando `ls -il`. A opção `-l` server para mostrar detalhes do arquivo, como as permissões. A opção -i mostra o número do inode, que é um identificador único para um arquivo ou diretório(como se fosse um cpf deles) no sistema de arquivos. O número do inode é importante porque ele é usado pelo sistema de arquivos para gerenciar os arquivos e diretórios. Ele é diferente para cada arquivo ou diretório, mesmo que eles tenham o mesmo nome. Execute:
```bash
